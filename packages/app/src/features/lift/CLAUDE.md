...
| `AccessoryTable.tsx`               | Accessory exercises table grouped by subtype (upper/lower/core/unclassified), each in its own collapsible section with last-performed summary; rendered for accessory tab only              |
...
| `useAccessoryTable.ts`             | Data aggregation for `AccessoryTable` (groups accessory rows by subtype in fixed order upper→lower→core→unclassified, filters empty groups, formats last-session summaries via `@dyel/api`) |

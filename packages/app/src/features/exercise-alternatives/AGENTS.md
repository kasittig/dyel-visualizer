...
| `useExerciseAlternatives.ts`          | Feature state adapter over `@dyel/api` recommendation values |
...
`useExerciseAlternatives.ts` owns exercise/category/direction selection state, resets downstream
choices, calls the standalone `@dyel/api` selectors, and returns display-ready grouped results.

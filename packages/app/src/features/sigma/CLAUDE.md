...
| `SigmaChart.tsx`               | Σ overview radar (3+ lifts) or pie chart (<3 lifts); wraps `BaseRadarChart`; logic owned by `useLatestLiftE1RMs` hook           |
| `useLatestLiftE1RMs.ts`        | Computes latest e1RM by lift type from chart data; wraps memoized `latestLiftE1RMs` derivation                                  |

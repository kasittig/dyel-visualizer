...
**Team history chart (normalized companion line):** `useTeamViewSelection`'s return value also includes a `normalizedPointsByLifter` field — per-lifter e1RM `Point[]` normalized to comp-lift equivalents via `normalizeTeamHistoryPoints` from `@dyel/api`, computed alongside `pointsByLifter` in the same refactored memo and filtered through the same date-range logic as the raw points. `TeamHistoryChart.tsx` renders one additional dashed, color-matched companion line per lifter (via the same `extraChildren` mechanism `ConjugateCharts` already uses for its normalized reference line), only when that lifter has normalized data — an unfitted canonical drops the point rather than falling back to raw. The companion line is labeled distinctly in the tooltip (e.g., `"<name> (normalized)"`) to distinguish it from the raw solid line, mirroring `ConjugateCharts`' existing single normalized reference line but applied per-lifter.

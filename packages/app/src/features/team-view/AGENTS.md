...
Team view for comparing a single exercise across multiple lifters. This feature is architecturally different from every other feature in the app: it does NOT use `PipelineContext`/`usePipelineModel()` at all, because it needs to fan out over MANY lifters' `PipelineModel`s simultaneously (one per row in the table) rather than the single model the rest of the app operates on via `PipelineContext`. The table is built from the shared `Table`/`TableCard`/`TableHeadRow`/`TableRow`/`TableCell` primitives in `shared/components/Table.tsx` (the same primitives `DiagnosticsPanel`/`AccessoryTable` use), with page-local modifier classes (`columnDivider`, `placeholderCell`, `controlCell`, `cellTint`) layered on via `TableCell`'s `className` prop. Columns are sortable via the shared `useSortableRows` hook, called directly in `TeamViewPage.tsx` (not `useTeamViewSelection.ts`, since it's pure display-ordering over already-derived rows) — mirroring the same pattern `AccessoryTable`/`DiagnosticsPanel` use; every column, including Exercise and Reps (which render a `TypeaheadDropdown`/`EffortPopover` per row for per-lifter overrides, sorted by `row.effectiveDisplayName`/`row.effectiveReps` respectively — clicking the header text sorts, independent of interacting with the control itself), is sortable, with the Target weight/e1RM columns' sort accessors matching whichever value (actual vs. projected) is currently displayed per row's `showProjected` toggle.
...
| `useTeamViewData.ts`          | Fetches the published index CSV and uses `loadIndexPipelineModels` from `@dyel/api` to concurrently load and parse each lifter's `PipelineModel`; returns status/data. Wraps both the index-CSV fetch and each per-lifter fetch with `cachedFetchSheetCsv` (`teamCsvCache.ts`) so repeat mounts (e.g. navigating between `/team` and `/team/summary`, which share this hook) reuse already-downloaded CSV text instead of re-fetching every lifter's sheet                                                                                                                                                                                                                                           |
| `teamCsvCache.ts`             | Read-through/write-through `localStorage` cache (`dyel:teamCsvCache`) mapping sheet URL → CSV text, used by `useTeamViewData.ts`'s `cachedFetchSheetCsv`. Simpler than `usePipelineOrchestration`'s single-sheet cache (`sheetCacheUtils.ts`) since it caches raw CSV text (JSON-safe, no `Date` round-tripping) rather than a built `PipelineModel`, and is keyed per-URL since this feature fans out over many sheets, not one                                                                                                                                                                                                                                                                     |
| `useTeamViewSelection.ts`     | Owns exercise/facet/date selection and event wiring. Related per-lifter overrides and projected toggles use one reducer; pure row calculations delegate to `@dyel/api`'s `buildTeamViewRowSnapshot`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `useTeamHistoryChartData.ts`  | Thin memoized wrapper calling `buildTeamHistoryChartData` from `@dyel/api`, used by `TeamHistoryChart`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `TeamHistoryChart.tsx`        | Presentation-only chart rendering one line per lifter for the currently-selected exercise, via the shared `MultiSeriesLineChart`; hidden by default, toggled by a button in `TeamViewPage.tsx`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `TeamHistoryChart.module.css` | Composes the shared chart card/section-label classes from `shared/charts/charts.module.css`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
`shared/components/AGENTS.md`). `e1rmSourceLabel` is built via the shared `formatE1RMSourceLabel`
...
`` `${siteRootPath()}?sheet=${encodeURIComponent(row.url)}` `` with `target="_blank"` — this
deep-links into the main DYEL Visualizer (`useAppSettings.ts`'s `sheet` query-param handling)
pre-loaded with that lifter's data, opened in a new tab so the viewer doesn't lose their place in
the comparison table. The "← Back to DYEL Visualizer" and "🏆 Summary view" links (just above it in
the same component) are built the same way. `siteRootPath()` (from `shared/pageRouting.ts`) strips
whatever known page path is currently matched off `window.location.pathname` and returns the app's
true root, e.g. `/` locally or `/dyel-visualizer/` on GitHub Pages — this replaced a dot-relative
convention (`href="."`/`href="./..."`) that looked correct for single-segment paths like `/team`
but broke for the two-segment `/team/summary` path: the browser resolves `"."` based on the
_current_ path's depth/trailing-slash, not the app's actual root, so from `/team/summary` it landed
on `/team/` instead — one level too shallow — creating a click-loop between the two pages instead
of ever reaching the root visualizer.

**Team history chart:** `useTeamViewSelection` derives a `pointsByLifter` field — per-lifter e1RM `Point[]` history already filtered to that lifter's effective canonical, reusing the same override-resolution inputs as `rows` but kept in a separate memo since it's chart-shaped data rather than table-row-shaped data. `TeamViewPage.tsx` manages local `useState` (`showHistoryChart`, default `false`) to toggle the chart's visibility; it's only shown once an exercise is selected, allowing viewers to see each lifter's e1RM progression over time alongside the static comparison table. The chart toggle button uses CSS classes `.graphButton`/`.graphButtonActive` for a visually prominent button appearance (label `"Graph"`, not a plain chip; active state communicates open/closed rather than text swapping). `TeamHistoryChart.tsx`'s card header includes a `DateRangePicker` component (backed by `dateRange`/`setDateRange` state in `useTeamViewSelection`, filtering `pointsByLifter` via `isRecordInDateRange` from `@dyel/api`) and a close (✕) button (wired to `onClose`, bound in `TeamViewPage.tsx` to `() => setShowHistoryChart(false)`); a `historySessionDates` memo (computed from unfiltered points) preserves full date availability for the picker regardless of the currently-applied range.

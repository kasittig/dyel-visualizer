  ...
  const rows = useMemo<TeamSummaryRow[]>(() => {
    ...
    return results.map((res): TeamSummaryRow => {
      ...
      return {
        ...
        url: res.url,
        hasData: true,
        squatDisplay: e1rms.squat !== undefined ? `${e1rms.squat} ${unit}` : '—',
        benchDisplay: e1rms.bench !== undefined ? `${e1rms.bench} ${unit}` : '—',
        deadliftDisplay: e1rms.deadlift !== undefined ? `${e1rms.deadlift} ${unit}` : '—',
        totalDisplay: e1rms.total !== undefined ? `${e1rms.total} ${unit}` : '—',

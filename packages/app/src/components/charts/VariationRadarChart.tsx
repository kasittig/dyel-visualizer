...
const renderCustomAxis = (props: CustomAxisTickProps): React.JSX.Element => {
  ...
  return (
    <Text x={offsetX} y={offsetY} textAnchor={textAnchor} fontSize={11}
...
export function VariationRadarChart({
  ...
  const data = useMemo(() => {
    ...
    return distinctDisplayNames(rows)
      .map((name) => {
        ...
        if (!sourceEx || !lastSess || !targetEx) return { variation: name, e1rm: undefined, targetE1rm };
        return {
          variation: name,
      ...
      .filter((d): d is { variation: string; e1rm: number; targetE1rm: number | undefined } => d.e1rm !== undefined);
  ...
  return (
    <section style={{ marginTop: '1rem' }}>
      <p style={{ fontSize: '0.8rem', color: 'var(--text)', marginBottom: '0.25rem', textAlign: 'center' }}>

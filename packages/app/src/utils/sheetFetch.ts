...
export function csvFetchError(status: number, contentType: string): string | null {
  if (status === 404) return "Sheet not found. Check that the URL is correct.";
  if (status === 401 || status === 403) return NOT_PUBLIC_MESSAGE;
  if (status === 400)
    return "Could not fetch this sheet. Make sure the sheet is published to the web as CSV (File → Share → Publish to web).";
  if (status < 200 || status >= 300) return `Unexpected error (HTTP ${status}).`;
  if (!contentType.includes("text/csv")) return NOT_PUBLIC_MESSAGE;
  return null;
}

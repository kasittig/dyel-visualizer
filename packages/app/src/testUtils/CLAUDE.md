...
5. For a series with a known, accepted divergence from the legacy `@dyel/core` implementation, write a "soft warn" test: `console.warn` on suspicious values instead of failing, but still assert the series is non-empty.
...
- `diffChartSeries` — two-array behavioral comparison: given a legacy output and a pipeline output over the same fixture, join them and assert the two implementations produce the same values (or document acceptable divergence).
...
4. Assert on the result:
   ...
   - For a series with a known acceptable divergence: `expect(diff.maxRelDiff).toBeLessThan(0.05)` (5% tolerance) and soft-warn if exceeded (e.g. `if (diff.maxRelDiff > 0.05) console.warn(...)`).

...
4. Exercise-type tabs (squat / bench / deadlift / accessory) and `LiftTabPanel` (which composes `ConjugateCharts` + `VariationRadarChart` + `DiagnosticsPanel`) consume the derived data from selectors.

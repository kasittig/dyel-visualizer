...
- **`textExtract.ts`** — `extractTextLines` splits a pasted `TextFieldInput` blob (`../types/TextFieldInput.ts`) into trimmed, non-empty lines, returning `string[] | null` (`null` when there are no usable lines). No domain parsing of line contents — that's `transform/parseTextData.ts`. Also exported from the `@dyel/core` public barrel since `packages/app` consumes it directly.

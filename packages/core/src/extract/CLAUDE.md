...
- **`textExtract.ts`** — `extractTextLines` splits a pasted `TextFieldInput` blob (`../types/TextFieldInput.ts`) into trimmed, non-empty lines, returning `string[] | null` (`null` when there are no usable lines). No domain parsing of line contents — that belongs in a future `transform/` function.

 ...
 * unchanged. First extracts an optional date token (anywhere in the line, via
 * `extractDateToken`) into the `date` key `parseSessionDate` already reads, then tries the
 * rep-max grammar (`"<exercise> <N>rm <weight><unit>"`) on the remainder, falling back to the
 * plain weight/reps grammar (`"<exercise> <weight><unit> x<reps>"`, reps optional).
 *
...
export function textLineToRow(line: string): RawRow | null {
  ...
  if (tokens.length >= 3) {
    ...
    if (repMaxMatch && weightMatch) {
      ...
      return { exercise, [key]: weight, ...dateField };
    }
  }

...
- **`parsers/`** — row/field-level parsing logic used by `parseConjugateData.ts`, `parseTextData.ts`, and `validateSheetCsv.ts`. See its own `CLAUDE.md`.

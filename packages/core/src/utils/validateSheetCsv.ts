...
export function validateSheetCsv(csv: string): SheetValidationResult {
  ...
  if (!hasExercise) issues.push("Missing required column: 'exercise'");
  if (!hasDate) issues.push("Missing required column: 'date'");
  if (!hasWeight)
    issues.push("Missing required column: 'weight' — add a 'weight (lbs)' or 'weight (kg)' column");
  if (!hasReps) issues.push("Missing required column: 'reps'");
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!exerciseName) rowProblems.push("Exercise name is empty");

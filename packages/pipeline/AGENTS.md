...
- **`src/pipeline.ts`** — the orchestrator. `runPipelineModel(raw, athlete, now?): PipelineModel` runs the full parse → tag → fit-normalization → derive-points → diagnose sequence once. Diagnostics consume `e1rm-max-effort` points so a newer dynamic-effort volume day cannot replace the latest qualifying strength observation; actual chain/band e1RM remains in bar-weight terms and the estimated additional-weight offset is subtracted from the fitted expectation before comparison. `buildDatasetsFromModel(model, specs, ui)` turns a model into chart-ready rows without re-running the pipeline. `PipelineModel.points` is the single point-query boundary: `points.get(deriver, { groupBy, adjusted })`; canonical/label and adjusted variants remain precomputed internally. `PipelineModel.tagged` exposes per-set detail. The normalization model is fit from competition-lift records, while point data includes competition and accessory records.

...
- **`src/pipeline.ts`** — the orchestrator. `runPipelineModel(raw, athlete, now?): PipelineModel` runs the full parse → tag → fit-normalization → derive-points → diagnose sequence once, first deriving the athlete's stronger deadlift stance (sumo vs. conventional, by e1RM projected forward to `now` via `derive/normalize.ts`'s `projectE1RMToDate` — not raw all-time best, so a stale PR in one stance can't outrank current strength in the other; ties/no-data defaulting to conventional) and patching a `'competition'` tag onto matching records before any other tag-dependent computation. `buildDatasetsFromModel(model, specs, ui)` turns a `PipelineModel` into chart-ready `RechartsRow[]` per spec without re-running the pipeline; `runPipeline(...)` is a convenience wrapper calling both in sequence. `PipelineModel` also exposes `tagged: TaggedSetRecord[]` for callers needing per-set (not day-collapsed) detail. **Perf invariant:** `groupByDateAndCanonical` and `groupByDateAndLabel` groupings are precomputed once via `Map.groupBy` and reused across all `pointsBy*` builders; `offsetAdjustRecords` runs once (not per-deriver-id), avoiding redundant O(records) passes. **Comp-lift vs. full-tagged scoping:** the normalization _model_ (`fitNormalizationModel`) is fit from comp-lift-only tagged records (`compTagged`) — baseline/variant-factor fitting only ever makes sense for the three comp lifts. `pointsByDeriver`/`pointsByLabelByDeriver` (and their offset-adjusted counterparts), however, are built from the FULL tagged set (`compTagged` + accessory records, via `tag/tag.ts`'s `buildAccessoryTaggedRecords`/`classifyAccessorySubtypes`), so tag-filtered specs like `lift:accessory` (consumed by `packages/app`'s Accessories tab) actually resolve to real point data instead of always-empty. Derivers are lift-type-agnostic; `diagnose` already treats any canonical with no fitted `variantFactor` (true for every accessory canonical) as `unassessed` rather than erroring.

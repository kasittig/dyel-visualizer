...
A React/Vite single-page app that visualizes conjugate powerlifting data from a Google Sheet. Drive it by starting the Vite dev server then running `.claude/skills/run-dyel-visualizer/driver.mjs` (Playwright-based) to navigate, interact, and screenshot. All paths below are relative to the repo root.
...
node .claude/skills/run-dyel-visualizer/driver.mjs

# Options:
node .claude/skills/run-dyel-visualizer/driver.mjs --out /tmp/shots        # custom output dir
node .claude/skills/run-dyel-visualizer/driver.mjs --page index            # index page (?page=index)
node .claude/skills/run-dyel-visualizer/driver.mjs --page conjugate        # conjugate info page
node .claude/skills/run-dyel-visualizer/driver.mjs --sheet-url "https://..." # pass a sheet URL
```
...
const { chromium } = createRequire(import.meta.url)(
  '/path/to/dyel-visualizer/.claude/skills/run-dyel-visualizer/node_modules/playwright'
);
...
- **`Cannot find package 'playwright'`**: The skill's `node_modules` is missing. Run `npm install` inside `.claude/skills/run-dyel-visualizer/` to install it there.
- **`EADDRINUSE` on port 5173**: Another dev server is running. Run `npx kill-port 5173` before starting.
- **`ERR_MODULE_NOT_FOUND` for the driver itself**: Run from the repo root — `node .claude/skills/run-dyel-visualizer/driver.mjs` — not from inside the skill directory.

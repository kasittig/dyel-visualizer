...
2. **`migration/ConjugateCharts.md`** — pipeline-native replacement validated
   ...
   `LiftTabPanel` (#5). Tracked: [#459](https://github.com/kasittig/dyel-visualizer/issues/459).
...
3. **`migration/VariationRadarChart.md`** — pipeline-native replacement
   ...
   dependency of `LiftTabPanel` (#5). Tracked:
   [#460](https://github.com/kasittig/dyel-visualizer/issues/460).

4. **`migration/RepCalculator.md`** and **`migration/StrengthScoreCalculator.md`** —
   independent of everything else above; both have a validated pipeline-native replacement and
   passing parity test, component not yet swapped:
   - `RepCalculator.tsx` still calls `findBestE1RM`/`buildSessionStats` (`@dyel/core`); swap to
     the existing `usePipelineRepCalculator` hook + `findBestE1RMFromPipeline`.
   - `StrengthScoreCalculator.tsx` still calls `calculateMetrics` (`@dyel/core`); swap to the
     existing `computeStrengthScores` (`@dyel/pipeline`) — same signature, one-line change.

5. **`migration/LiftTabPanel.md`** — composition root, last. Explicitly blocked on #1
...
Items #1 and #4 (`DiagnosticsPanel`, `RepCalculator`/`StrengthScoreCalculator`) have no
dependency on each other and can run concurrently. #2 and #3 are sequential (VariationRadarChart
depends on ConjugateCharts). #5 (`LiftTabPanel`) must come last regardless of how the rest is
split.

...
### 4. Schwartz-Malone female coefficient typo (non-monotonic table entry) — ✅ FIXED

- **File:** `packages/api/src/strengthScores.ts` (`SCHWARTZ_MALONE_FEMALE`)
- In `SCHWARTZ_MALONE_FEMALE`, the bodyweight-237 entry (`c: 0.5765`) was larger than both neighbors (236 → `0.5738`, 238 → `0.5754`) in an otherwise strictly decreasing table.
- **Root cause found (deeper than the original write-up assumed):** the anomaly wasn't a single mistyped digit at row 237. `git log -S`/`git show 45a13a9:packages/core/coefficients.json` recovered the original, OCR-verified, PDF-sourced female table from commit `45a13a9` ("Fix calculateSchwartzMalone: replace sparse anchor table with reference per-pound table") — that table has **zero** monotonicity violations across all 161 rows (w 90-250). Diffing it against the current table showed rows w=110-236 were silently re-transcribed to different (mostly lower) values during a later commit (`df15c89`/`c42d62d`, "Migrate Phase 1 components off `@dyel/core` to `@dyel/pipeline`") with no documented source, while rows w=237-250 were left untouched and still match `45a13a9` exactly. That undocumented re-transcription is what created the 236→237 inversion (236's drifted value dipped below both its true value and 238's untouched value) — no single-row interpolation could have fixed it, since a value satisfying `236 ≥ x ≥ 238` was mathematically impossible once 236 (`0.5738`) < 238 (`0.5754`).
